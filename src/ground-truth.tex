...
Let \(T\) be a set with cardinality \(m\). \\
...
\[ c \in C = c \text{ is an element in } C \]
\[ c \in C,\enspace \mathrm{idx}(c) = \text{the index of } c \text{ in } C \]
...
\[ C(\{u_1, u_2, \cdots\} \subseteq U) = (\enspace C(u_1),\enspace C(u_2)\enspace \cdots,\enspace ), \quad \text{preserving element order of } C \]
...
\[ \mathcal{R} \overset{\mathrm{row}}{\in} A = \mathcal{R} \text{ is a row in } A  \text{ as an } m \text{-tuple} \]
\[ \mathcal{C} \overset{\mathrm{col}}{\in} A = \mathcal{C} \text{ is a column in } A  \text{ as an } n \text{-tuple} \]
\[ \mathcal{R} \overset{\mathrm{row}}{\in} A,\enspace \mathrm{idx}(\mathcal{R}) = \text{the row index of } \mathcal{R}  \text{ in } A \]
\[ \mathcal{C} \overset{\mathrm{col}}{\in} A,\enspace \mathrm{idx}(\mathcal{C}) = \text{the column index of } \mathcal{C}  \text{ in } A \]
...
\[
	A(\{s_1, s_2, \cdots\} \subseteq S, \cdot) = \left[
	\begin{array}{c}
		A(s_1, \cdot) \\
		A(s_2, \cdot) \\
		\vdots
...
\[ A(\cdot, \{t_1, t_2, \cdots\} \subseteq T) = \left[\enspace A(\cdot, t_1)\enspace A(\cdot, t_2)\enspace \cdots\enspace \right],\quad \text{preserving column order of } A \]

\[ A(s, t) = A(s, \cdot)(t) = A(\cdot, t)(s) \]
\[ A(S', t) = A(S', \cdot)(\cdot, t) = A(\cdot, t)(S') \]
\[ A(s, T') = A(s, \cdot)(T') = A(\cdot, T')(s, \cdot) \]
